...
namespace ComputingVetoCore
{
    public class MPM
    {
        ...
        internal static bool IsInnerNode(int node)
        {
        ...
        public class Graph
        {
            ...
            {
                return OutCapacities.ContainsKey(vertex) ? OutCapacities[vertex].Count : 0;
            }

            internal int InDegree(int vertex)
            {
                return InCapacities.ContainsKey(vertex) ? InCapacities[vertex].Count : 0;
            }
            ...
            internal int GetEdgeCapacity(int start, int end)
            {
                return OutCapacities[start][end];
            }
            ...
            internal void Push(
                ...
                List<int> trace)
            {
                PushOrPull(node, amount, flow, trace, pushing: true);
            }
            ...
            internal void Pull(
                ...
                List<int> trace)
            {
                PushOrPull(node, amount, flow, trace, pushing: false);
            }
